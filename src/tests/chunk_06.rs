...
async fn linuxdo_credit_recharge_entitlement_starts_from_payment_month() {
    ...
    proxy
        .apply_linuxdo_credit_recharge_payment(&order.out_trade_no, "trade-payment-month", "ok=1", payment_month + 60)
        .await
    ...
    let months: Vec<i64> = audit.entitlements.iter().map(|entry| entry.month_start).collect();
